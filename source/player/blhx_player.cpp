...
void BlhxPlayer::RegisterSimilarOprations() {
  ...
  auto click_point_func = [](const std::vector<Element> &elements, float x,
    ...
    click_operation.type = PlayOperation::Type::SCREEN_CLICK;
  ...
  for (int chapter_idx = 1; chapter_idx <= chapter_num; ++chapter_idx) {
    for (int section_idx = 1; section_idx <= section_num; ++section_idx) {
      ...
      // Click chapter-section
      RegisterSimilarOpration(
          common_mode_name, GetCommonPageName(chapter_idx),
          std::bind(click_section_func, std::placeholders::_1,
                    chapter_section_name));
      RegisterSimilarOpration(
          hard_mode_name, GetHardPageName(chapter_idx),
          std::bind(click_section_func, std::placeholders::_1,
                    chapter_section_name));
      ...
      for (int before_chapter_idx = 1; before_chapter_idx < chapter_idx;
        ...
        RegisterSimilarOpration(
            common_mode_name, GetCommonPageName(before_chapter_idx),
            std::bind(click_point_func, std::placeholders::_1, next_chapter_x,
                      next_chapter_y));
        RegisterSimilarOpration(
            hard_mode_name, GetHardPageName(before_chapter_idx),
            std::bind(click_point_func, std::placeholders::_1, next_chapter_x,
                      next_chapter_y));
      }
      ...
      for (int behind_chapter_idx = chapter_idx + 1;
        ...
        RegisterSimilarOpration(
            common_mode_name, GetCommonPageName(behind_chapter_idx),
            std::bind(click_point_func, std::placeholders::_1, prev_chapter_x,
                      prev_chapter_y));
        RegisterSimilarOpration(
            hard_mode_name, GetHardPageName(behind_chapter_idx),
            std::bind(click_point_func, std::placeholders::_1, prev_chapter_x,
                      prev_chapter_y));
      }
    }
