...
std::vector<std::vector<float>> NMS(
    const std::vector<std::vector<float>> &boxes, float thresh) {
...
std::string Base64Encode(const uint8_t *data, unsigned long data_size) {
  ...
  for (unsigned long idx = 0; idx < data_size; ++idx) {
    input_chars_3.push_back(data[idx]);
    if (input_chars_3.size() == 3) {
      output_chars_4[0] = (input_chars_3[0] & 0xfc) >> 2;
      output_chars_4[1] =
          ((input_chars_3[0] & 0x03) << 4) + ((input_chars_3[1] & 0xf0) >> 4);
      output_chars_4[2] =
          ((input_chars_3[1] & 0x0f) << 2) + ((input_chars_3[2] & 0xc0) >> 6);
